...
declaration:
    T_INTEGER T_ID T_SEMICOLON
    {
		...
		symbol_table[*$2] = var_data( d_loc__.first_line, integer, false, false );
        delete $2;
    }
|
    T_BOOLEAN T_ID T_SEMICOLON
    {
		...
		symbol_table[*$2] = var_data( d_loc__.first_line, boolean, false, false );
        delete $2;
    }
|
    T_CONST T_INTEGER T_ID T_SEMICOLON
    {
		...
		symbol_table[*$3] = var_data( d_loc__.first_line, integer, true, false );
        delete $3;
    }
|
    T_CONST T_BOOLEAN T_ID T_SEMICOLON
    {
		...
		symbol_table[*$3] = var_data( d_loc__.first_line, boolean, true, false );
        delete $3;
    }
;
...
assignment:
    T_ID T_ASSIGN expression T_SEMICOLON
    {
        ...
        if(symbol_table[*$1].is_const == true && symbol_table[*$1].is_assigned == true)
        {
            std::stringstream ss;
		    ss << d_loc__.first_line << ": Const multi assignment error." << std::endl;
		    error( ss.str().c_str() );
        }
        if(symbol_table[*$1].is_const == true && symbol_table[*$1].is_assigned == false)
        {
